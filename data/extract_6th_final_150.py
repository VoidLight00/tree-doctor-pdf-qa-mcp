...
class ExamExtractor:
    ...
    def extract_text_combined(self, image: np.ndarray) -> str:
        ...
        # Tesseract 시도
        text_tesseract = self.extract_text_tesseract(enhanced)
    ...
    def process_page(self, page_num: int, image: Image.Image) -> List[Dict]:
        ...
        # 텍스트 추출
        text = self.extract_text_combined(img_array)
    ...
    def extract_all_questions(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            future_to_page = {
                executor.submit(self.process_page, i+1, img): i+1 
    ...
    def post_process_questions(self):
        ...
        if missing_numbers:
            ...
            # 누락된 문제 재시도
            for page_num in range(1, 50):  # 대략적인 페이지 수
                page_questions = [q for q in self.questions if q['page'] == page_num]
                if not page_questions:
                    continue
                
                page_numbers = {q['number'] for q in page_questions}
                for num in missing_numbers:
                    # 해당 페이지에 있을 가능성이 있는 문제인지 확인
                    if any(abs(num - pn) <= 5 for pn in page_numbers):
                        logger.info(f"페이지 {page_num}에서 문제 {num} 재검색")
                        # TODO: 해당 페이지 재처리
