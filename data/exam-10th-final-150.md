...
① 빠무이 : 성충으로 월동하며 열매예-알을 낳는다. (연 1회 발생, 성충으로 월동, 새잎이 나오기 시작하는 5~6 월에 새눈에 산란) (
...
① 솔잎혹파리는 집합페로몬트랩으로 예찰하여 방제시기를 결정한다.
...
① 오존과 『4씨은 황산화물과 탄화수소의 광화학 반응으로 발생한다. (
...
③ 농약의 효과는 살포량에 비례하기 때문에 많은 양을 살포할수록 효과는 계속 증가한다. @ 무인멀티콤터로 농약을 살포할 때 기류의 영향을 크게 받기 때문에 주변으로 비산되는 것을 주의해야 한다. 희석살포용 농약의 경우 정해진 희석배율로 조제하여 살포하지 않으면 약효가 저하되거나 약해가 유발 될 수 있다 #니톡 턱 3 농약와 휴과는 하가 때문애 많은 왕을 살포활수록 츄과는 계속 6가한다. (능 약의 효과는 특정 한 계점 이하에서는 비례하나, 한계점 이후로는 살포량을 증가할수록 효과가 점차 떨어진다.) 농약 살포 방법
...
① <희석살포제> - 가루형태 : 수용제[5『), 수화제(\『), 수화성미분제(\『) - 입상형태 : 입상수용제[수용성입제, 56), 입상수화제[(\/ㅇㅎㅇ) - 알 형태 : 정제상수화제[(\/1) - 액체형태 : 미탁제[\/6), 분산성액제[ㅁ0ㅇㄴ), 액상수화제[5ㅇ), 액제[9Ｌ), 오일제(2Ｌ), 유제[ㄷㅇ), 유상수화제(ㅇㅁㅁ), 유탁제[ㄷ\/), 유현탁제[55ㄷ), 캡숄현탁제[(ㅇ5) - 미생물 제제 : 고상제(6①, 액제(9Ｌ), 액상현탁제(5\/)
② 유제 (
③ 미탁제 @) 수용제
⑤ 액상수화제 <직접살포제> - 가루형태 : 미립제(\/ㅇ), 미분제(ㅎ『ㅁ), 분의제(05), 분제(
...
① ① 메트코나졸(01660ㅇㅁㅇ2016) [막에서 스테롤 생합성 저해 (사1 - 탈메탈 효소 기능 저해)] (
② 테부코나졸\{6640ㅇㅇㅁ0ㅇ2016) (
③ 펜피라자민[160ㅁㅁ\『02010106) (
④ 마이클로뷰타닐[0ㅁㅠ\여0ㅁ나ㅇㅁ1!)
⑤ 피라클로스트로빈[6『ㅇ여05\『ㅇㅁ1
...
## 120. 살충제 설폭사플로르[5.110×아10「)의 작용기작은?
...
## 121. 글루포시네이트암모늄 + 티아페나실 액상수화제의 유효성분별 작용기작을 옮게 나열한 것은?

① + 광계 || 저해[105) (
② 글루타민 합성효소 저해(11
③ 글루타민 합성효소 저해[11
④ (④ 아세틸 64 카르복실화 효소 저해[101) + 글루타민 합성효소 저해[11
⑤ 엽록소 생합성 저해[「11


---

## 122. 농약의 대사과정 중 복합기능 산화효 넌 에 터 소[071×60 +4001100 0×10056)가 관여하는 반응이 아닌 것은?

① 에폭시화 (
② -탈알킬화 (
③ 방향족 수 산화 (
④ 니트로기의 아민 변환
⑤ 산소 원자의 황 원자 치환 이 문제는 난이도 상급의 문제로 설명에 오류가 있을 수 있습니다. 감안하여 확인하시기 바랍니다.^^ '복합기능 산화효소 : 다양한 종류의 화합물을 산화하는ㅁ 화합물을 동시에 처리할 수 있는 능력을 가지고 있는 효소 - 농약이나 다른 화학물질을 인간이나 동물의 체내에서 비활성화하거나 더 쉽게 배출되도록 변환. - 농약에 대한 생체 대사 및 생리적 작용, 해독 과정에서 핵심적인 역할. 도 용되는 효소의 그룹으로 이러한 효소들은 여러 가지 는
...
## 124. '산림보호법 시행령' 제12조의10에 따른 나무병원 등록의 취소 또는 영업정지의 세부기준에 관한 설 명으로 올지 하으 것으? 6 ㄴ ㄴㄴ

① 부정한 방법으로 나무병원 등록을 변경한 경우 등록이 취소된다
② 나무병원 등록 기준에 미치지 못하는 경우
③ 차 위반 시 등록이 취소된다
④ 부정한 방법으로 나무병원 등록을 변경한 경우 등록이 취소된다
...
## 125. '산림보호법 시행규칙' 제19조의? (진료부, 처방전등의 서식 등)에 따라 나무의사가 작성하는 진료부 에 명시되지 않은 항목은

① 생육환경 (
② 진단결과 (
③ 수목의 표시 (
④ 수목의 상태
⑤ 처방 처치 등 치료방법 산렵보호법 시행규칙 [별지 제10호외17서식] =개첨 2


---

## 126. 수목 병해 방제법 중 생물적 방제에 해당하는 것은?
...
## 128. '소나무재선중병 방제지침' 소나무류 보존 가치가 큰 산림 중 '소나무 보호' 육성을 위한 법적 관리지 역에 포함되지 않는 것은?

④ , 123-
⑤ 금강소나무림 등 특별수종육성권 (1) 고요 ㅁ [국립공원은 '소나무재선중병 방제지침' 중, '소나무 보호 및 육성 관리지역'에 포함되어 있지 않음) 다음페이지 참고.


---

## 129. 산림 생태계의 천이에 관한 설명으로 옳은 것은?
...
## 130. 수목의 증산작용에 영향을 미치는 요인으로 옳지 않은 것은?
...
## 132. 산림토양의 질소 순환에 관한 설명으로 옳지 않은 것은?
...
## 135. 수목 해충의 천적 이용에 관한 설명으로 옳지 않은 것은?
...
## 137. 수목의 영양 진단법에 관한 설명으로 옳지 않은 것은?
...
## 138. 산림 병해충의 종합적 방제(IPM)에 관한 설명으로 옳은 것은?
...
## 139. 수목의 공기정화 기능에 관한 설명으로 옳지 않은 것은?
...
## 141. 수목의 도시 환경 스트레스에 관한 설명으로 옳은 것은?
...
## 142. 산림의 수원함양 기능에 관한 설명으로 옳지 않은 것은?
...
## 143. 수목 생장에 필요한 필수원소에 관한 설명으로 옳은 것은?
...
## 144. 산림 생태계의 먹이사슬에 관한 설명으로 옳지 않은 것은?
...
## 145. 수목의 상처 치유 과정에 관한 설명으로 옳은 것은?
...
## 146. 산림의 생물다양성 보전에 관한 설명으로 옳지 않은 것은?
...
## 147. 수목의 뿌리와 토양 미생물의 상호작용에 관한 설명으로 옳은 것은?
...
## 148. 도시 가로수 관리에 관한 설명으로 옳지 않은 것은?
...
## 149. 산림의 미기후 조절 기능에 관한 설명으로 옳은 것은?
...
## 150. 수목의 노화와 쇠퇴에 관한 설명으로 옳지 않은 것은?
